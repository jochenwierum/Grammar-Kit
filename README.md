...
This project is a fork of [JetBrains Grammar-Kit](https://github.com/JetBrains/grammar-kit/). I use Grammar-Kit and its `light-psi` version for personal playground projects, so please don't expect production readiness!

This version adds a gradle task to generate the `light-psi-all.jar` file. The file is generated by the following invocation:

```
./gradlew :buildLightpsi
```

The command generates the file `binaries/light-psi-all.jar`.

In addition, this version patches the LightPsi class to work with Java 11 and includes the classes `ASTWrapperPsiElement` and `GeneratedParserUtilBase` into `light-psi-all.jar`.

To use the generated class, the following gradle snipped can be used as a first approach:

```
dependencies {
    api files('libs/light-psi-all.jar', 'libs/grammar-kit-2021.1.3.jar')
    implementation group: 'org.jetbrains.intellij.deps', name: 'trove4j', version: '1.0.20200330'
    implementation group: 'jdom', name: 'jdom', version: '1.0'
    implementation group: 'com.google.guava', name: 'guava', version: '30.1-jre'
    implementation group: 'it.unimi.dsi', name: 'fastutil', version: '8.5.2'
}
```

Again, please note: The file `light-psi-all.jar` includes all classes that I need for my experiments! YMMV!
